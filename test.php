...
<?php
    ...
    if(isset($_GET['test'])){
        ...
        echo "<head>
                ...
                //  mysqli_query($conn,"SELECT * FROM subject JOIN assessment ON subject.subj_code = assessment.subj_code WHERE subject.subj_name = '$subject'");
                // mysqli_query($conn,"SELECT * FROM `$lang` WHERE tests = '$test'")
            if($result1 = mysqli_query($conn,"SELECT * FROM assessment JOIN question ON assessment.assessment_id = question.assessment_id WHERE assessment.assessment_name = '$test'")){
                    ...
                    <div class="oq-viewTests" id="test">
                        <div class="oq-instruct">
                            ...
                            <p id="instructions">Instructions are..<br>1. To select an option say option with number
                                example "option 3".<br>2. To repeat question say the word "repeat question".<br>3. To
                                navigate between questions say the word question and number example "question 3".<br>4.
                                To submit the test say the word "submit".<br><br>Say start to start your exam with
                                timer<br></p><br>
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <form action="result.php" method="post" name="oqform">
                                    <?php
                                                        $i = 1;
                                                        // mysqli_query($conn,"SELECT * FROM assessment JOIN question ON assessment.assessment_id = question.assessment_id WHERE assessment.assessment_name = '$test'")
                                                        //mysqli_query($conn,"SELECT * FROM `$testtitle`")
                                                        if($res = mysqli_query($conn,"SELECT * FROM assessment JOIN question ON assessment.assessment_id = question.assessment_id WHERE assessment.assessment_name = '$test'")){
                                                                ...
                                                                while($row = mysqli_fetch_assoc($res)){
                                                                    ...
                                                                    echo "
                                                                       ...
                                                                       echo "<input type='hidden' value='".$i."' id='quesNo'>";
                                                                        echo "<span class='oq-options'><input type='radio' name='qus".$i."' value='A' id='temp".$i."'> &nbsp;1) <span id='oq-options".$i.++$j."'>".nl2br($row['question_text'])."</span></span><br><br>";
