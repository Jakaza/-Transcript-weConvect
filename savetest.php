...
<?php
    ...
    if(isset($_GET['test'])){
        ...
        echo "<head>
                ...
                // $result1 = mysqli_query($conn,"SELECT * FROM `subject` WHERE tests = '$test'"  
            // $res=mysqli_query($conn,"SELECT * FROM subject JOIN assessment ON subject.subj_code = assessment.subj_code WHERE subject.subj_code = '$subj_code'");
                if($result1 =mysqli_query($conn,"SELECT * FROM assessment JOIN question ON assessment.assessment_id = question.assessment_id WHERE assessment.assessment_id = '$test'")){
                ...
                <div class="col-md-10 col-md-offset-1">
                    <div class="oq-viewTests" id="test">
                        <div class="oq-instruct">
                            <h3 id="testHead">Online Mock Exam - Test of <?php echo $test;?></h3><br>
                            <h3 id="instructHead">Say the word instructions or instruction to listen instructions</h3>
                            <br><br>
                            <p id="spresult7" class="spresult7"></p>
                            <p id="spresult8" class="spresult8"></p>
                            <p id="spresult" class="spresult"></p>
                            <p id="instructions">Instructions are <br>1. The questions will be spoken out, you need to
                                select the correct option by saying its option number. example if the correct answer is
                                option 2 then you need to say option 2 then that perticular option will be selected.
                                <br>2. To repeate a question you need to say the word repeat question.<br>3. You can
                                even navigate between questions by saying the question numbers example, question 2 then
                                question 2 will be spoken.<br>4. You can submit any time while taking the quiz to submit
                                you need to say submit, your test will be submitted and resutls are shown.<br><br>Say
                                start to start your exam with timer<br>
                            </p><br>
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <form action="result.php" method="post" name="oqform">
                                    <?php
                                                        $i = 1;
                                                        // mysqli_query($conn,"SELECT * FROM assessment JOIN question ON assessment.assessment_id = question.assessment_id WHERE assessment.assessment_id = '$test'")
                                                        //mysqli_query($conn,"SELECT * FROM `$testtitle`")
                                                        if($res = mysqli_query($conn,"SELECT * FROM assessment JOIN question ON assessment.assessment_id = question.assessment_id WHERE assessment.assessment_id = '$test'")){
                                                            if(mysqli_num_rows($res)>0){
                                                                ...
                                                                while($row = mysqli_fetch_assoc($res)){
                                                                    ...
                                                                    echo "
                                                                        ...
                                                                        echo "<button class='oq-aLeft oq-questions oq-form-control' disabled>Question ".$i.") ".nl2br($row['question_text'])."</button><br>";
